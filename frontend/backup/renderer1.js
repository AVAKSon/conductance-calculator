...
document.getElementById('csvInput').addEventListener('change', async (e) => {
  ...
  const columns = [];
  for (let i = 2; i < allCols.length; i += 1) {
    columns.push(allCols[i]);
  }
  const rowNames = data.data.map(row => Object.values(row)[0]); // Assuming first column is row labels
  

  // Metadata display (C3:C8)
  const metadataDiv = document.getElementById('fileInfo');
  const metadataTable = data.data.slice(1, 5);
  let metadataRows = [];

  // Safely extract from Excel rows
  metadataRows = [
    { label: 'FAB', value: Object.values(metadataTable[0])[2] ?? '' },
    { label: 'DAREA', value: 'DAREA' },  // placeholder
...
// Render using the same grid layout
  let metadataHtml = '';
  ...
  columns.forEach((colName, index) => {
    ...
    btn.onclick = () => {
      ...
      // Show corresponding chamber table
  ...
  renderChamberTable(columns[0], data.data, 0); // default to first chamber
});
...
function renderChamberTable(colName, rows, chamberIndex) {
  ...
  html += '<tr>';  // Don't include <td> before loop
  for (let i = 0; i < partLabels.length; i++) {
    const val = rows[i + 1]?.[colName] ?? '';  // Adjust index as needed
...
//5) drag-n-drop файла в окно программы
//6) сделать кнопку run simulation пока что серой